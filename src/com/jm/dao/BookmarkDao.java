...
public class BookmarkDao {
	...
	public ArrayList<BookmarkDto> bookmarkList(int id) {
		...
		String sql = "SELECT k.kanban_icon_p, d.title, d.document_id, w.workspace_id, w.workspace_name, m.name "
		...
		for(int i = 0; i<b_docuIdList.size(); i++) {
			...
			try {
				...
				while(rs.next()) {
					...
					bDto.setTitle(title);
					bDto.setDocument_id(document_id);
					bDto.setWorkspace_id(workspace_id);
