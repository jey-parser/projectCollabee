...
<head>
	...
	<link href="CollaStyle/0updatebar1.css" rel="stylesheet" type="text/css">
...
%>				
				<div class="container1 read" documentId="<%= dto.getId()%>">
					<div class="wname">
						<a id="wname2" class="wname2 live" href="Controller?command=select_Workspace_Index&workspaceId=<%= dto.getW_id()%>&workspaceName=<%= dto.getWname()%>&loginId=<%=loginId%>" workspaceId="<%= dto.getW_id()%>"><%= dto.getWname()%></a>
					</div>
					<div class="title">
...
%>
		<a href="Controller?command=showNewDocument&documentId=<%= dto.getId()%>&workspaceId=<%= dto.getW_id()%>&workspaceName=<%= dto.getWname()%>&loginId=<%=loginId%>" title="<%= dto.getTitle()%>" class="d_title3"><%= dto.getTitle()%></a>
<%
	}
%>
						</div>
