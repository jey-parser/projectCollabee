...
<%
	...
	int loginId = (Integer)request.getAttribute("loginId");
	session.setAttribute("loginId", loginId);
	int corporation_id = (Integer)request.getAttribute("corporation_id");
...
<head>
	...
	<link href="CollaStyle/0CollabeeHome.css" rel="stylesheet" type="text/css">
	...
	$(function(){
		...
		$(".header_menu5").on("click", function(){ 
			$.ajax({   
				type:"post",
				url:"Controller?command=getPrivateId", //프라이빗 공간 아이디 찾기
				data:{
					"loginId":<%=loginId%>
				},
				datatype:"json",
		...
		$(document).on("keyup", ".w_search", function(){
			...
			$.ajax({
				...
				success:function(data){
					$(".folder1").empty();//먼저 비움
					$.each(data, function(index, item){//입력할때마다 불러옴
